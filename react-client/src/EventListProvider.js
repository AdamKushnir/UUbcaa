...
function EventListProvider({ children }) {
  ...
  async function handleLoad() {
    ...
    const response = await fetch(`http://localhost:3000/match/list`, {
      method: "GET",
    });
  ...
  async function handleCreate(dtoIn) {
    ...
    if (response.status == 200) {
      // setEventLoadObject((current) => {
      //   console.log('pushing into current data');
  ...
  async function handleUpdate(dtoIn) {
    setEventLoadObject((current) => ({ ...current, state: "pending" }));
    const response = await fetch(`http://localhost:3000/match/update`, {
      method: "POST",
  ...
  async function handleDelete(dtoIn) {
    setEventLoadObject((current) => ({ ...current, state: "pending" }));
    const response = await fetch(`http://localhost:3000/match/delete`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(dtoIn),
    });
  ...
  const value = {
    ...
    handlerMap: { handleCreate, handleUpdate, handleAttendance, handleDelete },
  };
